...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "tcp") == 0) {
            ...
            printf("Usage:\n  %s [tcp|tcppi|rtu] - Mc client for unit testing\n\n", argv[0]);
            exit(1);
        }
    } else {
        /* By default */
    ...
    /* Single */
    rc = mc_write_bit(ctx, UT_BITS_ADDRESS, ON);
    printf("1/2 mc_write_bit: ");
    if (rc == 1) {
        printf("OK\n");
    } else {
        printf("FAILED\n");
    ...
    rc = mc_read_bits(ctx, UT_BITS_ADDRESS, 1, tab_rp_bits);
    printf("2/2 mc_read_bits: ");
    if (rc != 1) {
    ...
    /* Multiple bits */
    {
        ...
        mc_set_bits_from_bytes(tab_value, 0, UT_BITS_NB, UT_BITS_TAB);
        rc = mc_write_bits(ctx, UT_BITS_ADDRESS,
                               UT_BITS_NB, tab_value);
        printf("1/2 mc_write_bits: ");
        if (rc == UT_BITS_NB) {
            printf("OK\n");
        } else {
            printf("FAILED\n");
    ...
    rc = mc_read_bits(ctx, UT_BITS_ADDRESS, UT_BITS_NB, tab_rp_bits);
    printf("2/2 mc_read_bits: ");
    if (rc != UT_BITS_NB) {
        printf("FAILED (nb points %d)\n", rc);
        goto close;
    ...
    while (nb_points > 0) {
        ...
        value = mc_get_byte_from_bits(tab_rp_bits, i*8, nb_bits);
    ...
    /** DISCRETE INPUTS **/
    rc = mc_read_input_bits(ctx, UT_INPUT_BITS_ADDRESS,
                                UT_INPUT_BITS_NB, tab_rp_bits);
    printf("1/1 mc_read_input_bits: ");
    ...
    /* Single register */
    rc = mc_write_register(ctx, UT_REGISTERS_ADDRESS, 0x1234);
    printf("1/2 mc_write_register: ");
    if (rc == 1) {
        printf("OK\n");
    } else {
        printf("FAILED\n");
    ...
    rc = mc_read_registers(ctx, UT_REGISTERS_ADDRESS,
                               1, tab_rp_registers);
    printf("2/2 mc_read_registers: ");
    if (rc != 1) {
        printf("FAILED (nb points %d)\n", rc);
        goto close;
    }
    ...
    /* Many registers */
    rc = mc_write_registers(ctx, UT_REGISTERS_ADDRESS,
                                UT_REGISTERS_NB, UT_REGISTERS_TAB);
    printf("1/5 mc_write_registers: ");
    ...
    rc = mc_read_registers(ctx, UT_REGISTERS_ADDRESS,
                               0, tab_rp_registers);
    printf("3/5 mc_read_registers (0): ");
    ...
    rc = mc_write_and_read_registers(ctx,
                                         ...
                                         UT_REGISTERS_NB,
                                         tab_rp_registers);
    printf("4/5 mc_write_and_read_registers: ");
    if (rc != UT_REGISTERS_NB) {
    ...
    /** INPUT REGISTERS **/
    rc = mc_read_input_registers(ctx, UT_INPUT_REGISTERS_ADDRESS,
                                     UT_INPUT_REGISTERS_NB,
                                     tab_rp_registers);
    printf("1/1 mc_read_input_registers: ");
    ...
    real = mc_get_float(tab_rp_registers);
    if (real == UT_REAL) {
        printf("OK\n");
    } else {
        printf("FAILED (%f != %f)\n", real, UT_REAL);
    ...
    rc = mc_read_bits(ctx, UT_BITS_ADDRESS,
    ...
    rc = mc_read_input_registers(ctx, UT_INPUT_REGISTERS_ADDRESS,
                                     ...
                                     tab_rp_registers);
    printf("* mc_read_input_registers: ");
    if (rc == -1 && errno == EMBXILADD)
        printf("OK\n");
    else {
        printf("FAILED\n");
    ...
    rc = mc_write_bit(ctx, UT_BITS_ADDRESS + UT_BITS_NB, ON);
    printf("* mc_write_bit: ");
    if (rc == -1 && errno == EMBXILADD) {
        printf("OK\n");
    } else {
    ...
    rc = mc_write_bits(ctx, UT_BITS_ADDRESS + UT_BITS_NB,
                           UT_BITS_NB, tab_rp_bits);
    printf("* mc_write_coils: ");
    if (rc == -1 && errno == EMBXILADD) {
        printf("OK\n");
    } else {
        printf("FAILED\n");
    ...
    rc = mc_write_registers(ctx, UT_REGISTERS_ADDRESS + UT_REGISTERS_NB,
                                UT_REGISTERS_NB, tab_rp_registers);
    printf("* mc_write_registers: ");
    if (rc == -1 && errno == EMBXILADD) {
        printf("OK\n");
    } else {
        printf("FAILED\n");
    ...
    rc = mc_read_bits(ctx, UT_BITS_ADDRESS,
                          MC_MAX_READ_BITS + 1, tab_rp_bits);
    printf("* mc_read_bits: ");
    if (rc == -1 && errno == EMBMDATA) {
        printf("OK\n");
    } else {
    ...
    rc = mc_write_bits(ctx, UT_BITS_ADDRESS,
                           MC_MAX_WRITE_BITS + 1, tab_rp_bits);
    printf("* mc_write_bits: ");
    if (rc == -1 && errno == EMBMDATA) {
        printf("OK\n");
    } else {
        goto close;
    ...
    /** SLAVE REPLY **/
    printf("\nTEST SLAVE REPLY:\n");
    mc_set_slave(ctx, INVALID_SERVER_ID);
    rc = mc_read_registers(ctx, UT_REGISTERS_ADDRESS,
                               UT_REGISTERS_NB, tab_rp_registers);
    if (use_backend == RTU) {
        ...
        mc_send_raw_request(ctx, raw_req,
                                RAW_REQ_LENGTH * sizeof(uint8_t));
        rc = mc_receive_confirmation(ctx, rsp);
    ...
    rc = mc_set_slave(ctx, MC_BROADCAST_ADDRESS);
    if (rc == -1) {
    ...
    if (use_backend == RTU) {
        ...
        mc_set_slave(ctx, MC_TCP_SLAVE);
    }

    printf("3/4 Report slave ID: \n");
    /* tab_rp_bits is used to store bytes */
    rc = mc_report_slave_id(ctx, tab_rp_bits);
    if (rc == -1) {
        printf("FAILED\n");
        goto close;
    }

    /* Slave ID is an arbitraty number for libmc */
    if (rc > 0) {
    ...
    mc_set_response_timeout(ctx, &old_response_timeout);
    ...
    rc = mc_read_registers(ctx, UT_REGISTERS_ADDRESS_SPECIAL,
                               UT_REGISTERS_NB, tab_rp_registers);
    printf("* mc_read_registers at special address: ");
    ...
    printf("\nTEST RAW REQUEST:\n");
    {
        ...
        req_length = mc_send_raw_request(ctx, raw_req,
                                             RAW_REQ_LENGTH * sizeof(uint8_t));

        printf("* mc_send_raw_request: ");
        ...
        printf("* mc_receive_confirmation: ");
        rc  = mc_receive_confirmation(ctx, rsp);
