...
static int _mc_rtu_build_request_basis(mc_t *ctx, int function,
                                           int addr, int nb,
                                           uint8_t *req, int command, int subcommand)
{
    ...
    return _MC_RTU_PRESET_REQ_LENGTH;
}

/* Builds a RTU response header */
static int _mc_rtu_build_response_basis(sft_t *sft, uint8_t *rsp)
{
    /* In this case, the slave is certainly valid because a check is already
     * done in _mc_rtu_listen */
    rsp[0] = sft->slave;
    rsp[1] = sft->function;

    return _MC_RTU_PRESET_RSP_LENGTH;
}
...
int _mc_rtu_prepare_response_tid(const uint8_t *req, int *req_length)
{
    (*req_length) -= _MC_RTU_CHECKSUM_LENGTH;
    /* No TID */
    return 0;
}

int _mc_rtu_send_msg_pre(uint8_t *req, int req_length)
{
...
ssize_t _mc_rtu_send(mc_t *ctx, const uint8_t *req, int req_length)
{
#if defined(_WIN32)
    mc_rtu_t *ctx_rtu = ctx->backend_data;
    DWORD n_bytes = 0;
...
ssize_t _mc_rtu_recv(mc_t *ctx, uint8_t *rsp, int rsp_length)
{
#if defined(_WIN32)
    return win32_ser_read(&((mc_rtu_t *)ctx->backend_data)->w_ser, rsp, rsp_length);
#else
    return read(ctx->s, rsp, rsp_length);
#endif
}

int _mc_rtu_flush(mc_t *);
...
int _mc_rtu_check_integrity(mc_t *ctx, uint8_t *msg,
...
static int _mc_rtu_connect(mc_t *ctx)
{
#if defined(_WIN32)
    DCB dcb;
#else
    ...
    */
    ...
    ctx_rtu->serial_mode = MC_RTU_RS232;
#endif
...
{
    if (ctx->backend->backend_type == _MC_BACKEND_TYPE_RTU) {
#if HAVE_DECL_TIOCSRS485
        mc_rtu_t *ctx_rtu = ctx->backend_data;
        struct serial_rs485 rs485conf;
        memset(&rs485conf, 0x0, sizeof(struct serial_rs485));

        if (mode == MC_RTU_RS485) {
            rs485conf.flags = SER_RS485_ENABLED;
            if (ioctl(ctx->s, TIOCSRS485, &rs485conf) < 0) {
                return -1;
            }

            ctx_rtu->serial_mode |= MC_RTU_RS485;
            return 0;
        } else if (mode == MC_RTU_RS232) {
            if (ioctl(ctx->s, TIOCSRS485, &rs485conf) < 0) {
                return -1;
            }
...
int _mc_rtu_select(mc_t *ctx, fd_set *rfds,
                       struct timeval *tv, int length_to_read)
{
    int s_rc;
#if defined(_WIN32)
    s_rc = win32_ser_select(&(((mc_rtu_t*)ctx->backend_data)->w_ser),
                            length_to_read, tv);
    if (s_rc == 0) {
        errno = ETIMEDOUT;
        return -1;
...
int _mc_rtu_filter_request(mc_t *ctx, int slave)
{
    /* Filter on the Mc unit identifier (slave) in RTU mode */
    if (slave != ctx->slave && slave != MC_BROADCAST_ADDRESS) {
...
const mc_backend_t _mc_rtu_backend = {
    _MC_BACKEND_TYPE_RTU,
    _MC_RTU_HEADER_LENGTH,
    _MC_RTU_CHECKSUM_LENGTH,
    MC_RTU_MAX_ADU_LENGTH,
    _mc_set_slave,
    _mc_rtu_build_request_basis,
    _mc_rtu_build_response_basis,
    _mc_rtu_prepare_response_tid,
    _mc_rtu_send_msg_pre,
    _mc_rtu_send,
    _mc_rtu_recv,
    _mc_rtu_check_integrity,
    NULL,
    _mc_rtu_connect,
    _mc_rtu_close,
    _mc_rtu_flush,
    _mc_rtu_select,
    _mc_rtu_filter_request
};

mc_t* mc_new_rtu(const char *device,
                         ...
                         int stop_bit)
{
    ...
    ctx->backend = &_mc_rtu_backend;
    ctx->backend_data = (mc_rtu_t *) malloc(sizeof(mc_rtu_t));
    ctx_rtu = (mc_rtu_t *)ctx->backend_data;
