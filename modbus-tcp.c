...
static int _modbus_set_slave(modbus_t *ctx, int slave)
{
    ...
    if (slave >= 0 && slave <= 247) {
        ctx->slave = slave;
    } else if (slave == MODBUS_TCP_SLAVE) {           /*孔--MODBUS_TCP_SLAVE宏定义为0xFF*/
...
//                                    uint8_t *req)
int _modbus_tcp_build_request_basis(modbus_t *ctx, int function,    //增加形参指令和子指令
										int addr, int nb,
...

{
    ...
    //return _MODBUS_TCP_PRESET_REQ_LENGTH;       /*孔--modbus TCP当前请求预设长度，定义为12，为什么是12？？？？*/
...
int _modbus_tcp_build_response_basis(sft_t *sft, uint8_t *rsp)
{
    ...
    return _MODBUS_TCP_PRESET_RSP_LENGTH;        /*孔--modbus TCP当前回复预设长度，定义为8，为什么是8？？？？*/
}
...
const modbus_backend_t _modbus_tcp_backend = {
    _MODBUS_BACKEND_TYPE_TCP,           /*孔--宏定义为1*/
    _MODBUS_TCP_HEADER_LENGTH,          /*孔--宏定义为7*/
    _MODBUS_TCP_CHECKSUM_LENGTH,        /*孔--宏定义为0*/
    MODBUS_TCP_MAX_ADU_LENGTH,          /*孔--宏定义为260*/
    _modbus_set_slave,
...
{
    modbus_t *ctx;                       /*孔--这个结构体内容很多*/
    modbus_tcp_t *ctx_tcp;               /*孔--这个结构体只有port和address*/
    ...
    ctx = (modbus_t *) malloc(sizeof(modbus_t));     /*孔--开辟modbus_t长度的动态内存区，将地址转换*/
    _modbus_init_common(ctx);                        /*孔--成modbus_t*型，放在ctx里，并给这个地址里的*/
                                                     /*孔--元素进行赋值*/
    /* Could be changed after to reach a remote serial Modbus device */
    ctx->slave = MODBUS_TCP_SLAVE;           /*孔--MODBUS_TCP_SLAVE宏定义为0xFF，覆盖初值*/

    ctx->backend = &(_modbus_tcp_backend);   /*孔--用上面定义的_modbus_tcp_backend为开辟的空间的元素赋值*/

    ctx->backend_data = (modbus_tcp_t *) malloc(sizeof(modbus_tcp_t));  
                                                     /*孔--开辟modbus_tcp_t长度的动态内存区，将地址转换*/
	                                                 /*孔--成modbus_tcp_t*型，放在ctx->backend_data里*/
                                                     /*孔--为何上面是用的&，而下面是一个指针？因为上面是*/
	                                                 /*孔--一个实实在在的结构体，下面只是一个类型，没有实例*/
													 
    ctx_tcp = (modbus_tcp_t *)ctx->backend_data;     /*孔--‘->’的优先级高于*，所以是将上面的指针转换成*/
                                                     /*孔--modbus_tcp_t*类型，并赋值*/
