...
# define MODBUS_BEGIN_DECLS  extern "C" {      /*孔--如果是C++编译程序，则被花括号括起来的部分按照C语言进行编译*/
# define MODBUS_END_DECLS    }                 /*孔--若不是C++，则花括号的地方变成空*/
#else 
...
#define MODBUS_ENOBASE 112345678                 /*孔--随机数，但为什么是112345678？？？*/ 

/* Protocol exceptions */
enum {
    MODBUS_EXCEPTION_ILLEGAL_FUNCTION = 0x01,    /*孔--枚举类型默认以后从第一个值依次递增1*/ 
    MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS,       /*孔--所以这个值为0x02*/ 
...
} modbus_mapping_t;         /* 孔--定义结构体modbus_mapping_t，包含以上元素*/

typedef enum
{
    MODBUS_ERROR_RECOVERY_NONE          = 0,
    MODBUS_ERROR_RECOVERY_LINK          = (1<<1),      /*孔--1左移1位是2，实际上就是*2，比*2快*/
    MODBUS_ERROR_RECOVERY_PROTOCOL      = (1<<2),      /*孔--1左移2位是4*/
} modbus_error_recovery_mode;          /* 孔--重命名一个enum为modbus_error_recovery_mode，包含以上元素*/
...
const char *modbus_strerror(int errnum);            /*本函数为返回指针值的函数*/
...
#define MODBUS_GET_HIGH_BYTE(data) (((data) >> 8) & 0xFF)   /*孔--data右移8位，然后按位与，得到高字节*/
#define MODBUS_GET_LOW_BYTE(data) ((data) & 0xFF)            /*孔--data按位与，得到低字节*/
...
#include "modbus-tcp.h"                                      /*孔--跑这才包含这两个头文件*/
