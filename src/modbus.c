...
#include "modbus.h"
#include "modbus-private.h"         /*孔-- .h里面包含了modbus-version.h,这里面包含了modbus-private.h  */

/* Internal use */
#define MSG_LENGTH_UNDEFINED -1

/* Exported version */
const unsigned int libmodbus_version_major = LIBMODBUS_VERSION_MAJOR;    /*孔-- 是否这个值现在为（3）？？？ */
...
const char *modbus_strerror(int errnum) {             /* 孔--定义函数内容，声明是在modbus.h里。errnum是形参*/
                              ...
                              uint8_t *rsp, int rsp_length)
{
    ...
    if (ctx->backend->pre_check_confirmation) {
        ...
        if (rc == -1) {
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) { //孔--错误恢复模式并且确实错误了
                _sleep_and_flush(ctx);
            }
                          ...
                          uint16_t *dest)
{
    ...
    uint8_t rsp[MAX_MESSAGE_LENGTH];     //孔--最大消息长度定义为260，rsp[260],这只是中间存放的数组，最终响应
                                         //孔--在dest里面
    if (nb > MODBUS_MAX_READ_REGISTERS) {   /*孔--最大读寄存器长度定义为125*/
        if (ctx->debug) {
            fprintf(stderr,
...
int modbus_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest)
{
    ...
    if (nb > MODBUS_MAX_READ_REGISTERS) {                /* 孔--如果数量大于最大数量，且调试，则打印输出*/
        if (ctx->debug) {
            fprintf(stderr,
                    "ERROR Too many registers requested (%d > %d)\n",
                    nb, MODBUS_MAX_READ_REGISTERS);
...
void _modbus_init_common(modbus_t *ctx)
{
    ...
    ctx->debug = FALSE;
    ctx->error_recovery = MODBUS_ERROR_RECOVERY_NONE;    /*孔--默认定义为0*/
